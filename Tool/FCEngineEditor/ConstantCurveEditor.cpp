...
ConstantCurveEditor::ConstantCurveEditor()
{
}
...
bool ConstantCurveEditor::GetPropertyCtrlValueFromControl( wxVariant& variant, wxPGProperty* property, wxWindow* ctrl )
{
    bool bRes = true;
    wxTextCtrl* pTextCtrl = wxStaticCast(ctrl, wxTextCtrl);
    wxString textVal = pTextCtrl->GetValue();
    ConstantCurveProperty* p = (ConstantCurveProperty*)property;
    if (p->GetValueType() == ConstantCurveProperty::ConstantRandom_Type)
    {
...
bool ConstantCurveEditor::GetValueFromControl( wxVariant& variant, wxPGProperty* property, wxWindow* ctrl ) const
{
    return GetTextCtrlValueFromControl(variant, property, ctrl);
}

void ConstantCurveEditor::DrawValue(wxDC &dc, const wxRect &rect, wxPGProperty *property, const wxString &text) const
{
    ConstantCurveProperty* p = (ConstantCurveProperty*)property;
    wxSize drawSize;
    drawSize.x = rect.x + wxPG_XBEFORETEXT - DEFAULT_IMAGE_OFFSET_INCREMENT;
    drawSize.y = rect.y;
    if (p->GetValueType() == ConstantCurveProperty::Constant_Type)
    {
        dc.DrawText( text, drawSize.x, drawSize.y );
    }
    else if (p->GetValueType() == ConstantCurveProperty::ConstantRandom_Type)
    {
        int iX1 = drawSize.x;
...
wxPGWindowList ConstantCurveEditor::CreateControls( wxPropertyGrid* pPropGrid,
                                                           ...
                                                           const wxSize& sz ) const
{
    ...
    wxPGWindowList list;
    ConstantCurveProperty* p = (ConstantCurveProperty*)pProperty;
    ...
    else if(p->GetValueType() == ConstantCurveProperty::Curve_Type)
    {
    ...
    else if(p->GetValueType() == ConstantCurveProperty::CurveRandom_Type)
    {
