...
CMD5::CMD5(const std::string &str) 
{
    Reset();
    Update(str);
}

CMD5::CMD5(const std::wstring &str)
{
    Reset();
    Update(str);
}
...
void CMD5::Reset() 
{
    ...
    m_bitsCount[0] = m_bitsCount[1] = 0;
    /* Load magic initialization constants. */
    m_magicNumber[0] = 0x67452301;
    m_magicNumber[1] = 0xefcdab89;
    m_magicNumber[2] = 0x98badcfe;
    m_magicNumber[3] = 0x10325476;
}

const BYTE* CMD5::GetData() const
{
    return (const BYTE*)(&m_digest);
}

// use const_cast to make sure the ref CMD5 has done Digest() operation.
bool CMD5::operator==( const CMD5& rRef )
{
    CMD5* pRef = const_cast<CMD5*>(&rRef);
    std::string rRefString = pRef->ToString();
    
    return rRefString == ToString();
}
...
void CMD5::Update(FILE* in)
{
    BEATS_ASSERT(in != NULL);
    if (in != NULL)
    {
        long long lastPos = _ftelli64(in);
...
void CMD5::Update(const BYTE *input, size_t length) 
{
    ...
    if(length >= partLen) 
    {
        memcpy(&m_inputBuffer[index], input, partLen);
    ...
    memcpy(&m_inputBuffer[index], &input[i], length-i);
}
...
void CMD5::Digest() 
{
    ...
    Encode(m_magicNumber, m_digest, 16);

    /* Restore current state and count. */
    memcpy(m_magicNumber, oldState, 16);
    memcpy(m_bitsCount, oldCount, 8);
    m_bDigested = true;
}
