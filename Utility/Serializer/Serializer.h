...
{
public:
    CSerializer(uint32_t size = DefaultSerializerBufferSize, void* data = NULL);
    ...
    CSerializer(FILE* pFile, uint32_t startPos = 0, uint32_t dataLength = 0);
    ...
    bool Serialize(FILE* pFile, uint32_t startPos = 0, uint32_t dataLength = 0);
    ...
    void ValidateBuffer(uint32_t size);
